...
test("references establish membership and support empty regions/subregions and full Markdown", () => {
  ...
  assert.equal(archive.regions[0].subregions[0].villages[0].id, "village");
...
test("schema rejects unknown fields, unsafe paths, invalid coordinates and missing parents", () => {
  ...
  assert.throws(() => parseArchiveFiles(new Map([["regions/region/region.json", jsonText({ names, color: "red" })]])), /color/u);
  assert.throws(() => parseArchiveFiles(new Map([["regions/region/region.json", jsonText({ names, color: "#336699", id: "region" })]])), /exactly/u);
  ...
  assert.throws(() => parseArchiveFiles(new Map([["villages/village/info.en.md", "notes"]])), /Missing/u);
});
...
test("flat migration preserves map output and every Markdown byte", async (t) => {
  const { readArchiveFiles: readNested } = await import("../scripts/nested-archive.js");
...
test("village moves preserve IDs and notes, validate references, and detect source/destination conflicts", async () => {
  ...
  assert.equal(parseArchiveFiles(moved).places[0].id, "stable-id");
  assert.equal(parseArchiveFiles(moved).places[0].regionId, "second");
  assert.equal(parseArchiveFiles(moved).regions.find(r => r.id === "second").subregions[0].villages.length, 1);
  ...
  assert.equal(parseArchiveFiles(direct).regions.find(r => r.id === "second").villages.length, 1);
});
