...
import { ContentError, jsonText, parseContentFiles, recordType, validateMetadata } from "./content.js";
...
export function changeFiles(files, change) {
  ...
  if (change.action === "delete") {
    ...
    const children = parseContentFiles(files).records.filter(record => type === "region"
...
export function checkConflict(baseFiles, latestFiles, change) {
  ...
  if (change.action === "delete" && type !== "village") {
    ...
    for (const files of [baseFiles, latestFiles]) {
      for (const record of parseContentFiles(files).records) {
  ...
  if ([...keys].some((key) => relevant(key) && baseFiles.get(key) !== latestFiles.get(key))) {
    throw new ContentError("This record or its region/subregion changed since you opened it. Your input has been kept. Reload the latest map content and review your edits before trying again.", 409);
  }
}
