...
import { readContentFiles } from "../lib/content.js";
...
const contentFiles = await readContentFiles(new URL("../info", import.meta.url));
...
try {
  ...
  assert.equal((await (await fetch(`${base}/api/content`)).json()).revision, firstRevision, "Fresh startup initializes the empty mount from the local remote");
  const regionFile = [...contentFiles.keys()].find(name => name.startsWith("regions/"));
  const region = JSON.parse(contentFiles.get(regionFile)); region.names.en = "Compose content update";
  ...
  const updatedResponse = await fetch(`${base}/api/content`);
  const updatedContent = await updatedResponse.json();
  if (updatedContent.revision !== secondRevision) {
